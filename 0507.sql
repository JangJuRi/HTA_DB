...
-- 20000달러 이상은 전체 평균 급여의 10%을 보너스로, 10000달러 이상은 15%, 그 외는 20%를 보너스로 지급하려고 한다.
-- 아이디, 이름, 급여, 보너스 조회하기
select employee_id, first_name, salary,
    case
...
-- with 절
with
dept_costs AS   -- 부서이름, 부서 총 급여
(select B.department_name, sum(A.salary) as dept_total
 from employees A, departments B
 where A.department_id = B.department_id
 group by B.department_name),
avg_cost AS     -- 부서별 총 급여에 대한 평균 급여
(select sum(dept_total)/count(*) as dept_avg
 from dept_costs)
select *
from dept_costs
where dept_total > (select dept_avg     -- 부서 총 급여가 평균 급여
                    from avg_cost)
...
-- 101번 직원의 모든 부하 조회하기
select lpad(employee_id, level*4, ' ')employee_id, first_name, manager_id
from employees
